...
namespace PhysFS {
	...
	std::size_t base_fstream::length() {
	...
	ofstream::ofstream(const std::string& filename, mode writeMode)
	...
	fstream::fstream(const std::string& filename, mode openMode)
	...
	void setWriteDir(const std::string& newDir) {
		PHYSFS_setWriteDir(newDir.c_str());
	}

	void addToSearchPath(const std::string& newDir, bool append) {
		PHYSFS_addToSearchPath(newDir.c_str(), append ? 1 : 0);
	}

	void removeFromSearchPath(const std::string& oldDir) {
	...
	void setSaneConfig(const std::string& orgName, const std::string& appName,
		const std::string& archiveExt, bool includeCdRoms, bool archivesFirst) {
	...
	StringList enumerateFiles(const std::string& directory) {
		StringList files;
	...
	void enumerateFiles(const std::string& directory, EnumFilesCallback callback, void * extra) {
	...
	sint64 getLastModTime(const std::string& filename) {
	...
	void mount(const std::string& newDir, const std::string& mountPoint, bool appendToPath) {
	...
	std::string Util::utf8FromUcs4(const uint32* src) {
		std::string value;
	...
	std::string Util::utf8ToUcs4(const char* src) {
		std::string value;
	...
	std::string Util::utf8FromUcs2(const uint16* src) {
		std::string value;
		std::size_t length = strlen((char*)src);
		char * buffer = new char[length]; // will be smaller than len
	...
	std::string Util::utf8ToUcs2(const char* src) {
		std::string value;
	...
	std::string Util::utf8FromLatin1(const char* src) {
		std::string value;
