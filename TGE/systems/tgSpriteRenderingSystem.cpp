...
tgSpriteRenderingSystem::tgSpriteRenderingSystem(int screen_width, int screen_height)
	: m_vbo(nullptr),
	m_ibo(nullptr),
	m_vao(nullptr),
	m_prevVBOSize(0),
...
void tgSpriteRenderingSystem::resize(int w, int h) {
	m_projection = tgMatrix4::ortho(0, float(w), float(h), 0, -1, 1);
	updateUniforms();
}

void tgSpriteRenderingSystem::updateUniforms() {
	m_shader->bind();
	m_shader->setMatrix4("viewProjection", m_projection * m_view);
	m_shader->setInt("tex0", 0);
}

void tgSpriteRenderingSystem::updateBuffers() {
	if (m_sprites.size() == 0) {
		return;
	}
...
void tgSpriteRenderingSystem::begin() {
	if (!m_drawing) {
		m_drawing = true;
		m_shader->bind();
...
void tgSpriteRenderingSystem::end() {
	if (m_drawing) {
		m_drawing = false;
		updateBuffers();
		render();
...
void tgSpriteRenderingSystem::render() {
	m_vao->bind();
...
void tgSpriteRenderingSystem::drawTile(tgTexture *atlas, int tileIndex, int tileWidth, int tileHeight, int tileX, int tileY, float scale) {
